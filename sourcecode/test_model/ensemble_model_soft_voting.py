...
print('predicting')
ensemble_word2vec = VotingClassifier(estimators=[('lr',word2vec_models[0]),('svm',word2vec_models[1]),('rf',word2vec_models[2])],voting='soft',weights=[0.62167,0.6567,0.595],flatten_transform=True)
ensemble_word2vec.fit(vectors, labels)
ensemble_tfidf = VotingClassifier(estimators=[('lr',LogisticRegressionModeltfidf),('svc',SVCModeltfidf),('rf',RandomForestModeltfidf)],voting='soft',weights=[0.63,0.623,0.567],flatten_transform=True)
...
w_model_name = '../models_bin/ensemble_softvoting_with_word2vec.joblib'
dump(ensemble_word2vec,w_model_name) 

tf_model_name = '../models_bin/ensemble_softvoting_with_tf.joblib'
