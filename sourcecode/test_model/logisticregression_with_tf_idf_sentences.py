...
# params = [{'penalty':['l1'],'solver':['saga'],'multi_class':['multinomial','ovr']},{'penalty':['l2'],'solver':['lbfgs','newton-cg','sag'],'random_state':[0,2,4,6,8],'max_iter':[260,280,300],'multi_class':['multinomial','ovr']},{'penalty':['l1'],'solver':['liblinear'],'multi_class':['ovr']}]

# for score in scores:
#     LogisticRegressionModel = GridSearchCV(LogisticRegression(),params,cv=5,scoring='%s_macro' % score).fit(vectors,labels)


LogisticRegressionModel = LogisticRegression(penalty='l1',solver='saga',multi_class='ovr').fit(vectors,labels)
print('testing')
...
# print(LogisticRegressionModel.best_params_)
print("test result:")
# print(test_result)
print("--------------------------------------------------------------")
...
model_name = '../models_bin/losgisticRegression_with_tfidf.joblib'
dump(LogisticRegressionModel,model_name)


# for experiment only 
# link_exp = "experiments/lr_tf_exp_start%s_length_%s.txt"%(str(train_data_begin_index),(str((train_data_end_index)*3)))
# f = open(link_exp,"w+",encoding="utf-8")
# penalty=['l1','l2']
# solver=['liblinear','saga','lbfgs','newton-cg','sag']
# multi_class=['multinomial','ovr']
# random_state=[0,2,4,6,8]
# max_iter=[260,280,300]
